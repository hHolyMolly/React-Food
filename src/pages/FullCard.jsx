...
function FullCard() {
	...
	const [hiddenDescriptionCard, setHiddenDescriptionCard] = React.useState(false);
	...
	React.useEffect(() => {
		...
		if (descriptionHeight !== undefined && descriptionHeight.offsetHeight > 160) setHiddenDescriptionCard(true);
	}, [product]);
	...
	const findItemFavorite = favoriteItems.find((obj) => obj.parentId === Number(id));
	...
	const ButtonBasket = () => {
		if (!findItemBasket) {
			return (
				<Button className="product-card-footer__basket full-card-actions__basket" onClick={addedToBasket} orange>
					<span className="product-card-footer__basket-text">
						Купить
					</span>
					<i className="product-card-footer__basket-icon">
						<svg width="22" height="22" viewBox="0 0 26 26" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path d="M4.33337 20.5833C4.33337 21.158 4.56165 21.7091 4.96798 22.1154C5.3743 22.5217 5.9254 22.75 6.50004 22.75C7.07468 22.75 7.62578 22.5217 8.03211 22.1154C8.43843 21.7091 8.66671 21.158 8.66671 20.5833C8.66671 20.0087 8.43843 19.4576 8.03211 19.0513C7.62578 18.6449 7.07468 18.4167 6.50004 18.4167C5.9254 18.4167 5.3743 18.6449 4.96798 19.0513C4.56165 19.4576 4.33337 20.0087 4.33337 20.5833Z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
							<path d="M16.25 20.5833C16.25 21.158 16.4783 21.7091 16.8846 22.1154C17.2909 22.5217 17.842 22.75 18.4167 22.75C18.9913 22.75 19.5424 22.5217 19.9487 22.1154C20.3551 21.7091 20.5833 21.158 20.5833 20.5833C20.5833 20.0087 20.3551 19.4576 19.9487 19.0513C19.5424 18.6449 18.9913 18.4167 18.4167 18.4167C17.842 18.4167 17.2909 18.6449 16.8846 19.0513C16.4783 19.4576 16.25 20.0087 16.25 20.5833Z" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
			...
			return (
				<Button className="product-card-footer__basket full-card-actions__basket _disabled" href="/basket" tag="Link" orange>
					<span className="product-card-footer__basket-text">
						В корзине
					</span>
					<i className="product-card-footer__basket-icon">
					...
					<div className="full-card__body">
						{product === null ?
							...
							<div className="full-card__main full-card-main">
								{mobileCard && <h1 className="full-card-main__title _title">{product.title}</h1>}
								<div className="full-card-main__slider full-card-slider">
									...
									>
										{product.price.stock &&
											<div className="full-card-slider__label">
												Акция
											</div>}
										{product.images.map((slide, idx) => (
											...
											>
												<img src={`./img/sections/products/${category}/${slide.imageUrl}`} alt={product.title} />
											</SwiperSlide>
										))}
									</Swiper>
								...
								<div className="full-card-main__inner">
									...
									<div className="full-card-main__actions full-card-actions">
										<div className="full-card-actions__price">
											<strong className={classNames(
												...
												product.price.stock && "full-card-actions__price-text_stock"
											)}>{product.price.value}₴</strong>
											{product.price.stock && <strong className="full-card-actions__price-text">{product.price.stock}₴</strong>}
										</div>
										<ButtonBasket />
									...
									{product.contain &&
										<p className="full-card-main__contain">
											<span className="full-card-main__contain_bold">Состав:</span> {product.contain}
										</p>}
									<p className="full-card-main__weight">
										<span className="full-card-main__weight_dark">{product.weight}</span>
									</p>
									<div
										className={classNames(
									...
									>
										{product.description.map((item, idx) => (
											<p className="full-card-main__text" key={`product-description_${idx}`}>
												{item.text}
											</p>
										))}
									</div>
									{hiddenDescriptionCard &&
										<button className="full-card-main__description-button" onClick={showDescription}>
											Показать ещё
										</button>}
								</div>
							</div>
