...
function Header() {
	...
	const { isActive: searchActive } = useSelector(({ search }) => search);
	const { isActive: categoryActive } = useSelector(({ categories }) => categories);
	const { totalCount: totalBasketCount } = useSelector(({ basket }) => basket);
	const { totalCount: totalFavoriteCount } = useSelector(({ favorite }) => favorite);
	...
	const handleCategory = () => {
		!categoryActive ? dispatch(setOpenedCategory(true)) : dispatch(setOpenedCategory(false));
	};
							...
							<ul className="header__actions header-actions">
								...
								<li className="header-actions__item">
									<Link className="header-actions__link" to="/basket">
										...
										{totalBasketCount > 0 &&
											<span className="header-actions__value">
												{totalBasketCount > 9 ? "9" : totalBasketCount}
											</span>}
									</Link>
								</li>
							</ul>}
